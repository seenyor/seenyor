...
const OtpVerification = () => {
  ...
  const verifyCode = () => {
    ...
    if (code === decryptedOtp) {
      ...
      const encryptedOtp = CryptoJS.AES.encrypt(code, "your-secret-key").toString();
      router.push(`/login/set-password?email=${encodeURIComponent(encryptedEmail)}&otp=${encodeURIComponent(encryptedOtp)}`);
    } else {
  ...
  return (
    <div className="flex flex-col gap-[1.88rem] h-screen w-full justify-center items-center">
      <div className="flex flex-col gap-2 w-full md:justify-center items-center">
        <Heading size="heading7xl" as="h2" className="font-bold text-green-200">Check your email</Heading>
        <Text as="p" className="text-[1.5rem] sm:text-[1rem] font-normal leading-[1.69rem] text-body md:text-center sm:px-2">
          We&apos;ve emailed a 6-digit code to <span className="font-medium text-text">{email}</span>. Please use it soon.
        </Text>
      </div>
      ...
      <div className="flex flex-col items-center justify-center gap-6 w-full">
        ...
        <button onClick={verifyCode} className="px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark transition-colors">
          Verify Code
        </button>
      </div>
    </div>
